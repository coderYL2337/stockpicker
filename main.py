...
def main():
    ...
    query = st.text_input(
        label="Stock Search",
        placeholder="e.g., 'electric car manufacturers' or 'data center builders'",
...
#                     # Better version with inline ellipsis
#                     st.write("**Business Summary**")
#                     business_summary = stock['description']
#                     if len(business_summary) > 200:
#                         button_id = f"expand_summary_{stock['ticker']}"
#                         if st.session_state.get(button_id, False):
#                             # Show full text with collapse option
#                             st.write(business_summary)
#                             if st.button("Collapse ▲", key=f"collapse_{stock['ticker']}"):
#                                 st.session_state[button_id] = False
#                                 st.rerun()
#                         else:
#                             # Show preview with clickable ellipsis
#                             preview = f"{business_summary[:200]}"
#                             if st.button(f"{preview}...", key=f"expand_{stock['ticker']}", help="Click to show full text"):
#                                 st.session_state[button_id] = True
#                                 st.rerun()
#                     else:
#                         st.write(business_summary)
...
#                     # Metrics with smaller font
#                     # In display_stock_data function, update the metrics section:
#                     metrics_cols = st.columns(2)
#                     with metrics_cols[0]:
#                         st.markdown("**Current Price**")
#                         st.markdown(f"<span style='font-size: 16px'>${stock['realtime_data'].get('close', 0):.2f}</span>", unsafe_allow_html=True)
                        
#                         st.markdown("**Market Cap**")
#                         market_cap = stock['yfinance_data'].get('marketCap', 0)
#                         if market_cap >= 1e9:
...
#                         st.markdown(f"<span style='font-size: 16px'>{market_cap_str}</span>", unsafe_allow_html=True)
                        
#                         st.markdown("**Sector**")
#                         sector = st.session_state.stock_metadata[stock['ticker']].get('Sector', 'N/A') if st.session_state.stock_metadata else 'N/A'
#                         st.markdown(f"<span style='font-size: 16px'>{sector}</span>", unsafe_allow_html=True)
    
                        
#                         st.markdown("**P/E Ratio**")
#                         pe_ratio = stock['yfinance_data'].get('trailingPE', 0)
#                         st.markdown(f"<span style='font-size: 16px'>{pe_ratio:.2f}</span>", unsafe_allow_html=True)
                        
#                         st.markdown("**Beta**")
#                         beta = stock['yfinance_data'].get('beta', 0)
#                         st.markdown(f"<span style='font-size: 16px'>{beta:.2f}</span>", unsafe_allow_html=True)
                        
#                         st.markdown("**Earnings Growth**")
#                         st.markdown(f"<span style='font-size: 16px'>{stock['yfinance_data'].get('earningsGrowth', 0):.1f}%</span>", unsafe_allow_html=True)
...
#                         st.markdown(f"<span style='font-size: 16px'>{stock['yfinance_data'].get('revenueGrowth', 0):.1f}%</span>", unsafe_allow_html=True)
                        
#                         st.markdown("**Gross Margins**")
#                         st.markdown(f"<span style='font-size: 16px'>{stock['yfinance_data'].get('grossMargins', 0):.1f}%</span>", unsafe_allow_html=True)
                        
#                         st.markdown("**EBITDA Margins**")
#                         st.markdown(f"<span style='font-size: 16px'>{stock['yfinance_data'].get('ebitdaMargins', 0):.1f}%</span>", unsafe_allow_html=True)
                        
#                         st.markdown("**52 Week Change**")
#                         st.markdown(f"<span style='font-size: 16px'>{stock['yfinance_data'].get('52WeekChange', 0) * 100:.1f}%</span>", unsafe_allow_html=True)
...
#                     info_df = pd.DataFrame(stock_info_data)
#                     # csv_data = info_df.to_csv(index=False)
#                     csv_data = info_df.to_csv(index=False).encode('utf-8')
...
#                 if start_date and end_date:
#                     historical_data = get_all_historical_data(tickers, start_date, end_date)
#                     create_price_comparison_chart(historical_data)
                    
#                     analysis = generate_analysis_with_llm(all_stock_data)
#                     st.subheader("Stock Comparison Summary")
#                     st.write(analysis)
